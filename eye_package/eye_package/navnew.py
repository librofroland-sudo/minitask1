...
class NavNode(Node):
    def __init__(self):
        ...
        self.get_logger().info("nav_node started (with smarter obstacle avoidance).")
    ...
    def do_explore(self):
        ...
        if d_front < 0.35:
            ...
            if d_left > d_right:
                ...
                cmd.angular.z = random.choice([-1.0, 1.0])

            print("EMERGENCY: smart backing & turning in explore.")
        # 前方有障碍，但不至于贴脸 → 原地智能转向
        elif d_front < 0.7:
            cmd.linear.x = 0.0
            if d_left > d_right:
                ...
                cmd.angular.z = random.choice([-0.9, 0.9])
            print("Avoiding front obstacle in explore mode → smart left/right.")
        # 左右贴得太近时，稍微调整
        elif d_left < 0.4:
            cmd.linear.x = 0.10
            cmd.angular.z = -0.4
    ...
    def follow_global_path(self):
        ...
        if d_front < 0.35:
            ...
            if d_left > d_right:
                ...
                cmd.angular.z = random.choice([-1.0, 1.0])
            print("🔥 EMERGENCY while following path → smart backing.")
            self.cmd_pub.publish(cmd)
            return
        ...
        if d_front < 0.6:
            ...
            if d_left > d_right:
                ...
                cmd.angular.z = random.choice([-0.8, 0.8])
            print("⚠️ Avoiding obstacle while following path (smart left/right).")
            self.cmd_pub.publish(cmd)
            return
    ...
    def goto_target_direct(self):
        ...
        if d_front < 0.35:
            ...
            if d_left > d_right:
                ...
                cmd.angular.z = random.choice([-1.0, 1.0])
            print("🔥 EMERGENCY while going to target → smart backing!")
            self.cmd_pub.publish(cmd)
            return
        ...
        if d_front < 0.6:
            ...
            if d_left > d_right:
                ...
                cmd.angular.z = random.choice([-0.8, 0.8])
            print("⚠️ Avoiding obstacle before pursuing target (smart left/right).")
            self.cmd_pub.publish(cmd)
            return
